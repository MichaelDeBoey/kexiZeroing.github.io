...
Iterator helpers are chainable methods on iterator objects, not arrays. You need an iterator or a generator. Then you can build a lazy pipeline on top of it. You describe what you want, and the runtime pulls values only when needed.
...
Promise.all([loadImg("1.jpg"), loadImg("2.jpg"), loadImg("3.jpg")]).then(
  showImgs,
);
```
